define(['jquery', 'bootstrap', 'backend', 'addtabs', 'adminlte', 'form'], function($, undefined, Backend, undefined, AdminLTE, Form) {
    var Controller = {
        index: function() {
            ...
            if (storage) {
                $.post('tips/limit', function(res) {
                    if (res) {
                        var tips_time = storage.getItem('tips');
                        if (tips_time != null && tips_time == res.updatetime) {
                            ...
                            if (res.content) {
                                ...
                                storage.setItem('tips', res.updatetime)
                            }
                        }
        ...
        login: function() {
            ...
            $("#agentregister").click(function() {
                window.location.href = "{:url('index/agentregister')}";
                $("input[name=captcha]").next(".input-group-addon").find("img").trigger("click");
            });
        },
        agentregister: function() {
            ...
            $("#gettelcode").click(function() {
                var tel = $("#pd-form-agentname").val();
                if (tel == '') {
                    Toastr.error(__('Username can not be empty'));
                    return false;
                } else if (!(/^1[3456789]\d{9}$/.test(tel))) {
                    Toastr.error(__('Please fill in the correct mobile phone number'));
                    return false;
                } else {
                    Fast.api.ajax({
                        url: 'index/gettelcode',
                        data: {
                            tel: tel,
                            captcha: $('#captcha').val()
                        }
